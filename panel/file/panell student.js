...
const scriptURL = "https://script.google.com/macros/s/AKfycbyZ7e8Fn1MlqOpTsUYmWS9zi4li8XMns8UW2oLk332X6NL6AQHHaRz9OyMezKg2vjmQ/exec";
...
function login() {
  ...
  if (!username || !password) {
    loginMsg.innerText = "Please enter your username and password.";
    return;
  }

  // بررسی کپچا
  if (captchaInput !== document.getElementById("captchaCode").textContent) {
    loginMsg.innerText = "The security code is incorrect.";
  ...
  .then(data => {
    if (data.success) {
      ...
      loginMsg.innerText = "Login was successful!";
	  ...
	  document.getElementById("userNationalId").innerText = data.nationalId || "Uncertain";
document.getElementById("userFatherName").innerText = data.fatherName || "Uncertain";
document.getElementById("userLevel").innerText = data.level || "Uncertain";
...
const tabData = [
  {
    title: "Description",
    content: `<p class="text-sm text-gray-700 whitespace-pre-line">${session.description || "No explanation has been recorded."}</p>`
  },
...
<div id="${iframeId}-warning" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
  <div class="bg-white p-4 rounded-lg shadow-lg max-w-sm w-full">
    <h2 class="text-red-600 font-bold mb-2">Warning!</h2>
    <p class="text-sm text-gray-700 leading-6">
      Dear user ${userName}<br>
     If any copying (recording, copying, saving) of the panel content is observed, 
your panel will be permanently blocked.
    </p>
    ...
    `
    : `<p class="text-red-500">There is no video.</p>`
},
...
`
    : `<p class="text-red-500">There is no file</p>`
},
{
  title: "Downloads",
  content: `
    <ul class="list-disc pr-5 text-sm space-y-2">
      <li>
        ${session.fileIds
          ? `<a class="text-blue-600 underline" href="https://drive.google.com/uc?export=download&id=${session.fileIds}">Download the file</a>`
  ...
  if (idx === 0) tabContent.innerHTML = tab.content;
            ...
            p.textContent = "No sessions have been registered for this course.";
        ...
        li.textContent = "No active projects found.";
        coursesUl.appendChild(li);
      }

    } else {
      // اگر ورود موفق نبود، پیام خطا را نمایش بده
      loginMsg.innerText = data.message || "The login information is incorrect.";
    }
  })
  .catch(err => {
    // در صورت خطای ارتباط با سرور
    loginMsg.innerText = "Error communicating with the server.";
    console.error(err);
  });
}
